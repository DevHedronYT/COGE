...
void coge_gen_texture_t(coge_texture_t *tex) {
    ...
    if (strcmp(tex->m_path, "") != 0) {
        tex->m_buffer = stbi_load(tex->m_path, &tex->m_width, &tex->m_height,
                                  &tex->m_bpp, 4);
    }
    ...
    if (strcmp(tex->m_path, "") != 0) {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, tex->m_width, tex->m_height, 0,
                     GL_RGBA, GL_UNSIGNED_BYTE, tex->m_buffer);
    }

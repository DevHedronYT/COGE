...
coge_texture_t tex;
float vertices[20] = {-0.5f, -0.5f, 0.0f, 0.0f, 0.0f, 0.5f, -0.5f,
...
float movementVec[4] = {0, 0, 0, 0};

void Init() {
    window = coge_window_init(640, 480, "Dev Testing");
    shaderID = coge_load_shader("res/Shaders/vertex_core_one.glsl",
                                "res/Shaders/fragment_core_one.glsl");
    ...
    props = coge_gen_program_props_t(vertices, indices, sizeof(vertices),
                                     sizeof(indices), GL_STATIC_DRAW,
                                     GL_STATIC_DRAW);
    ...
    /*    long stride            = sizeof(Vertex);
          int pointers[4]        = { offsetof(Vertex, Position),
       offsetof(Vertex, Color), offsetof(Vertex, TexCoords), offsetof(Vertex,
       TexID) }; */
    ...
    for (i = 0; i < vertex_pointer_num; i++) {
        ...
        glVertexAttribPointer(index[i], size[i], GL_FLOAT, GL_FALSE, stride,
                              (void *)pointers[i]);
    }
...
void Update() {
    ...
    if (coge_keyboard_key_held_down(window, GLFW_KEY_LEFT) ||
        coge_keyboard_key_held_down(window, GLFW_KEY_A)) {
        movementVec[0] += 0.1;
    }

    if (coge_keyboard_key_held_down(window, GLFW_KEY_RIGHT) ||
        coge_keyboard_key_held_down(window, GLFW_KEY_D)) {
        movementVec[0] -= 0.1;
    }
    ...
    glUseProgram(shaderID);
    glUniform4f(uLocation, movementVec[0], movementVec[1], movementVec[2],
                movementVec[3]);
}
